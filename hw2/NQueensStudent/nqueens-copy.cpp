...
bool getSolutionsStk(vector<vector<int>> &board, vector<class Queen> &nqueens, int boardDim, int row) {
    ...
    int row = 1; // keep track of how many rows in the chessboard have been parsed
    bool solutions = true; // check if a queen was placed in the row of the current iteration
    
    while (row != boardDim) {
        if (recurStack.empty() && !solutions) { // exit condition: no solution row == 0
            break;
        }
        
        // Initialize a tempQueen to traverse through the row (start at the first col of the board)
        Queen tempQueen;
        tempQueen.row = row;
        tempQueen.col = 1;
        tempQueen.chessboard = board; // save a snapshot of the previous board
        
        bool isSafe = true; // check if a queen can be safely put onto a spot
        
        for (int i = 0; i < boardDim; i++) {
            if (board[row][i] <= -1) { // can't place queen on the path of another queen
                tempQueen.col++; // move queen to the right
                isSafe = false;
            }
            else if (board[row][i] == 1) { // a queen is already on this row
                isSafe = false;
                break; // exit the loop
            } 
            else if (board[row][i] == 0) { // found an open space on the row to place a queen
                isSafe = true;
                tempQueen.row = row;
                tempQueen.col = i;
        ...
        if (!isSafe) { // can't place a queen on the current row
            if (recurStack.empty()) { // if the stack is empty, then the queen was pre-placed on the board
...
//     while (rowCounter != boardDim) { // begin parsing rows to place a queen
// //         if (!solutions) {
// //              if (count(board[rowCounter].begin(), board[rowCounter].end(), 1)) {
// //                  rowCounter--;
// //                  solutions = true;
// //                  continue;
// //              }
...
//         Queen tempQueen; // initialize a tempQueen
//         tempQueen.chessboard = board; // save a snapshot of the board
        
//         // backtrack to previous queen and continue iterating through columns
//         if (!recurStack.empty() && !solutions) {
//             stacktop = recurStack.top();
            
//             tempQueen.row = stacktop.row;
//             tempQueen.col = stacktop.col + 1;
//             tempQueen.tempVar = false;
//             recurStack.pop();
//             recurStack.push(stacktop);
//         } else {
//             // Dummy traversal queen
//             tempQueen.row = row;
//             tempQueen.col = 1;      
//             tempQueen.tempVar = true;
            
//             recurStack.push(tempQueen);
//         }
        
//         for (i = tempQueen.col; i < boardDim; i++) { // loop through the columns in the given row
//             isSafe = true;

//             // TODO: Need to double check this logic
//             if (board[row][i] <= -1) { // can't place on an attack spot
//                 stacktop = recurStack.top();
//                 tempQueen.col = stacktop.col + 1; // move the queen to the right
//                 tempQueen.row = stacktop.row;
//                 tempQueen.chessboard = stacktop.chessboard;
//                 tempQueen.tempVar = stacktop.tempVar;
                
//                 recurStack.pop(); // pop the top off
//                 recurStack.push(tempQueen); // push updated tempQuen val
...
//                 stacktop = recurStack.top();
//                 tempQueen.col = i; // move the queen to the right
//                 tempQueen.row = row;
//                 tempQueen.chessboard = board;
//                 tempQueen.tempVar = false;
// //                 if(stacktop.tempVar == true) {
//                 recurStack.pop();
// //                 }
...
//                 recurStack.pop(); // remove temp queen
//                 isSafe = true;
//                 break;
//             }
//         }
...
//             solutions = true;
//             rowCounter++;
//             continue; // move onto the next row and continue to parse
//         } 
